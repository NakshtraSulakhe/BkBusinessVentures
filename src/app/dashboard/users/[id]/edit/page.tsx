...
function EditUserContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || 'Identity refinement failed.')
      }
    } catch (error) {
      console.error(error)
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-40 gap-4">
      ...
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic text-center">Querying Identity Registry...</p>
    </div>
  )
  ...
  return (
    <div className="space-y-6 animate-fade-in-up pb-20">
      <PageHeader
        title="Refine Identity Protocol"
        subtitle={`Adjusting authorization tiers for identity identifier ${id?.slice(0, 8)}`}
        actions={
          <Button
            variant="outline"
            ...
            <CardContent className="p-8">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Verified Name</Label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.isActive.toString()} onValueChange={(v) => setFormData({ ...formData, isActive: v === 'true' })}>
                      ...
                      <SelectContent className="rounded-2xl border-slate-200">
                        <SelectItem value="true">State: Verified Active</SelectItem>
                        <SelectItem value="false">State: Protocol Locked</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <div className="pt-6 flex justify-end">
                  ...
                  >
                    {submitting ? 'Refining...' : 'Commit Protocol Update'}
                  </Button>
                </div>
              </form>
            </CardContent>
          ...
          <Card className="bg-slate-900 border-none shadow-2xl rounded-[2.5rem] overflow-hidden">
            ...
            <CardContent className="p-8 pt-0 space-y-6">
              <p className="text-xs text-slate-400 font-bold leading-relaxed italic border-b border-white/10 pb-4">
                Changes to identity protocols are logged system-wide. Modification of the operational state will immediately restrict or restore portal access.
              </p>
              ...
              <div className="space-y-6">
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black text-rose-400 uppercase tracking-widest">State Lock Protocol</h5>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight mt-1 leading-relaxed">Transitioning to 'Protocol Locked' will terminate all active sessions for this identity identifier.</p>
                  </div>
                </div>
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h5 className="text-[10px] font-black text-blue-400 uppercase tracking-widest">Recursive Updates</h5>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight mt-1 leading-relaxed">Identity refinements propagate to all historically associated transaction signatures.</p>
                  </div>
                </div>
              </div>

              <div className="pt-6 border-t border-white/10 text-center">
                <Badge className="bg-white/5 text-slate-400 border-white/10 uppercase font-black text-[8px] tracking-widest mb-4">Registry Integrity Verified</Badge>
