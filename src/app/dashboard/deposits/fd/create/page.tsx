...
function CreateFDComponent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        showMessage('FD account created successfully', 'success')
        setTimeout(() => {
          router.push('/dashboard/deposits/fd')
        }, 2000)
      } else {
        const errorData = await response.json()
        showMessage(errorData.error || 'Failed to create FD account', 'error')
      }
    } catch (error) {
      console.error('Failed to create FD account:', error)
      showMessage('Failed to create FD account', 'error')
    } finally {
      setLoading(false)
    }
  }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: 'nearest' | 'up' | 'down') => 
                        setFormData(prev => ({ ...prev, roundingMode: value }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
